...
apt remove -y docker.io containerd kubelet kubeadm kubectl kubernetes-cni || true
apt autoremove -y
...
apt update -qq
apt -qq -y install podman cri-tools containers-common
...
echo "deb [signed-by=/etc/apt/keyrings/kubernetes-1-31-apt-keyring.gpg] https://pkgs.k8s.io/core:/stable:/v1.31/deb/ /" | sudo tee -a /etc/apt/source.list.d/kubernetes.list
echo "deb [signed-by=/etc/apt/keyrings/kubernetes-1-30-apt-keyring.gpg] https://pkgs.k8s.io/core:/stable:/v1.30/deb/ /" | sudo tee -a /etc/apt/source.list.d/kubernetes.list
apt --allow-unauthenticated update
apt --allow-unauthenticated install -y docker.io containerd kubelet=${KUBE_VERSION}-1.1 kubeadm=${KUBE_VERSION}-1.1 kubectl=${KUBE_VERSION}-1.1 kubernetes-cni
...
echo "THEN RUN THE OUTPUT AS A COMMAND HERE TO ADD AS WORKER"
echo