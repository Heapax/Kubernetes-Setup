...
### install packages
apt insatll -y apt-transport-https ca-certificates
...
echo "deb [signed-by=/etc/apt/keyrings/kubernetes-1-31-apt-keyring.gpg] https://pkgs.k8s.io/core:/stable:/v1.30/deb /" | sudo tee -a /etc/apt/source.list.d/kubernetes.list
apt --allow-unauthenticated update
